    ...
    <main class="bg-slate-900 flex flex-col justify-center items-center min-h-screen">
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            try {
                ...
                $mail->isHTML(true);
                $mail->Subject = 'Nowa wiadomosc ze strony acdc.net.pl';
                ...
                $thankYouMail->isSMTP();
                $thankYouMail->Host = $_ENV['SMTP_HOST'];
                $thankYouMail->SMTPAuth = true;
                $thankYouMail->Username = $_ENV['SMTP_USERNAME'];
                $thankYouMail->Password = $_ENV['SMTP_PASSWORD'];
                $thankYouMail->SMTPSecure = $_ENV['SMTP_SECURE'];
                $thankYouMail->Port = $_ENV['SMTP_PORT'];
                $thankYouMail->SMTPAutoTLS = false;

                $thankYouMail->setFrom($_ENV['SENDER_EMAIL'], 'acdc.net.pl');
                $thankYouMail->addAddress($email);

    

                $thankYouMail->isHTML(true);
                $thankYouMail->Subject = 'Dziekujemy za pytanie';

                $thankYouMessage = file_get_contents('thankyou_template.html');
                $thankYouMessage = str_replace('%messageContent%', $message, $thankYouMessage);
                $thankYouMail->Body = $thankYouMessage;

                $thankYouMail->addCustomHeader('Content-Type: text/html; charset=UTF-8');
                $thankYouMail->send();
                ...
                echo '<p class="text-3xl text-center text-white mt-4">Dziękujemy za wiadomość! Postaramy sie odpowiedzieć jak najszybciej</p>';
            } catch (Exception $e) {
                ...
                echo '<p class="text-3xl text-center text-white mt-4">Niestety coś poszło nie tak. Spróbuj sie z nami skontakować telefonicznie</p>';
            }
        }
        ?>
